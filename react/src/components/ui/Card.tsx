...
export function Card({ label, value, sub, accent, children, className = '' }: CardProps) {
  return (
    <div className={`relative overflow-hidden bg-bg2 border border-line rounded-md p-5 shadow-1 transition-all hover:border-line2 hover:-translate-y-0.5 hover:shadow-2 ${accent ? `after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 ${accentBar[accent]}` : ''} ${className}`}>
      {label && <div className="mono-label mb-2.5">{label}</div>}
      {value !== undefined && <div className="num font-semibold text-[1.95rem] leading-none text-ink mb-1.5">{value}</div>}
      {sub && <div className="font-mono text-[0.58rem] text-ink-dim tracking-wider">{sub}</div>}
      {children}
    </div>
  );
}
