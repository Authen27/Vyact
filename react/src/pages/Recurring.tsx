...
import { computeNextDueDate, buildRRule } from '../lib/recurring';

type SchedType = 'expense' | 'income' | 'investment';
type EndsKind = 'never' | 'count' | 'until';
...
export default function Recurring() {
  ...
  async function save() {
    ...
    // §5.2 — compose the RRULE from frequency + ends condition.
    const rfreq = freq === 'weekly' ? 'weekly' : freq === 'yearly' ? 'yearly' : 'monthly';
    const rrule = buildRRule(rfreq,
      endsKind === 'count' ? { kind: 'count', count: Math.max(1, parseInt(endsCount, 10) || 1) }
      : endsKind === 'until' && endsDate ? { kind: 'until', date: endsDate }
      : { kind: 'never' });
    const schedule = {
      ...(editing || {}),
      transactionTemplate: {
        ...
        // investments carry no category (v9 model).
    ...
    <div>
      <div className="flex justify-between items-start mb-5 gap-4">
        ...
        <button className="btn-primary flex-shrink-0" onClick={() => {
          ...
          setName('');
          setAmount('');
          setCategory('rent_mortgage');
          setDayOfMonth(1);
          setAutoConfirm(true);
          setReminderLead(3);
          setOwnerMemberId('');
          setEndsKind('never'); setEndsCount('12'); setEndsDate('');
        }}>+ Add Schedule</button>
      </div>
          ...
          : schedules.map(s => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-3 px-4 py-3 border-b border-line last:border-b-0">
                  ...
                  {/* §5 — the meaningless active/deactivate toggle is removed.
                     A schedule runs until its RRULE COUNT/UNTIL is exhausted or
                     it is deleted. */}
                  ...
                  <button onClick={() => {
                    ...
                    setEndsKind(s.rrule?.includes('COUNT=') ? 'count' : s.rrule?.includes('UNTIL=') ? 'until' : 'never');
                    setEndsCount(String((s.rrule?.match(/COUNT=(\d+)/) || [])[1] || '12'));
                    setEndsDate(((s.rrule?.match(/UNTIL=(\d{4})(\d{2})(\d{2})/) || []) as string[]).slice(1).join('-') || '');
      ...
      <Modal open={open} onClose={() => { setOpen(false); setEditing(null); }} title={editing ? 'Edit Recurring Schedule' : 'Add Recurring Schedule'}>
        <div>
          ...
          {/* §5.2 — ends condition → RRULE COUNT / UNTIL (mutually exclusive). */}
          <FieldRow>
            <Field label="Ends">
