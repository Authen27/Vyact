...
- Consumer: [`react/CHANGELOG.md`](react/CHANGELOG.md) — **current v10.13.0**
...
- **The invariant + golden suites are the gate:** `lib/__tests__/moneyModel.{invariants,regression,engines}.test.ts` — the §7 invariants (transfer/investment neutral, value-update = offset not txn, exact EMI split, balance fold, net worth = assets − liabilities, type-scoped categories) + the aggregation golden file. The test labels are off-by-one from the spec past INV-4 (mapping in the file header); spec INV-7 (atomicity) has no test. Update the snapshot **deliberately**, never blindly. Debt is already correct (EMI = interest expense + principal liability reduction) — do not "re-connect" or duplicate it.
