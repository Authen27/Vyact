...
**28 items total — 22 ✅ resolved · 4 ⚠ partial · 2 ⬜ open.**

| Bucket | IDs |
|---|---|
| ✅ **Resolved** (22) | TD-01, TD-03, TD-04, TD-05, TD-06, TD-07, TD-08, TD-09, TD-10, TD-11, TD-12, TD-13, TD-14, TD-15, TD-17, TD-18, TD-20, TD-21, **TD-23**, **TD-24**, **TD-26**, **TD-27** |
| ⚠ **Partial** (4) | TD-02 (unit + Lane-A done; integration layer open) · TD-19 (Lane A shipped; Lane B/RLS-isolation open) · TD-28 (CI version-drift guard shipped — closes TD-23's recurrence; CLAUDE.md trim + README docs-map still open) · **TD-25** (god-module eliminated — 8 slices + `store/index.ts` rename done, store.ts→`dataSlice.ts` 723; only the optional CRUD sub-split to hit ~300 remains) |
| ⬜ **Open** (2) | TD-16, TD-22 |
...
| TD-25 | ⚠ Consumer `store.ts` god-module → Zustand slices: 7 slices extracted (modal/reconcile/notify/recurring/cloudAuth/sync + shared localJson/testHooks) *(2026-06-15, 1,167→718)*; the data-core (init/refresh/CRUD) split + `index.ts` rename remain | Technical / Maintainability | High | L |
