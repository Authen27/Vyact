...
> notify, recurring, cloudAuth, sync, data, crud) + shared `localJson`/`testHooks`. `useStore`
> byte-identical throughout. All slices are <300 lines except **`dataSlice.ts` (559)** — the bootstrap
> (`init`/`refresh`) + the money-critical `upsertTransaction`/`recordDebtPayment` + profile/rates,
> deliberately kept as one cohesive unit (splitting the EMI/transfer money paths carries risk that
> outweighs the line-count target). Verified at every step with the money-model invariant + golden
> suites.
...
| TD-24 | ✅ Error opacity — `lib/faults.ts` taxonomy + silent-write-loss instrumentation + read-parse classification + dev `FaultsPanel` diagnostics *(resolved 2026-06-17; trivial storage-removeItem/decode noops intentionally terse per acceptance)* | Technical / Reliability | High | M |
| TD-25 | ✅ Consumer `store.ts` god-module → Zustand slices *(resolved 2026-06-17: 1,167-line store.ts → 35-line `store/index.ts` + 9 slices; useStore byte-identical; all <300 except `dataSlice` 559 = cohesive bootstrap+txn-money core)* | Technical / Maintainability | High | L |
...
**Resolution completed (2026-06-17).** Classified the read-corruption catches that carried real
diagnostic value — `localStorageCompat.readJson` and `kvStore.kvGet:memory` parse-failures now record
an `expected` fault (corrupt/non-JSON stored value) instead of silently returning null. Added the
in-app diagnostics view: [`components/dev/FaultsPanel.tsx`](react/src/components/dev/FaultsPanel.tsx) — a
collapsible floating panel over the `getFaults()` ring buffer showing recent `unexpected` faults
(dropped writes / contract violations / corruption), mounted in `App.tsx` behind `import.meta.env.DEV`
(never ships to prod). The remaining best-effort `catch {}` micro-sites (storage `removeItem`, the
base64 prefix decode, one-shot migrations) are **intentionally terse degraded paths** — the acceptance
explicitly allows "UI-cosmetic catches may stay terse," and none is a write/contract path. A registered
production transport (Sentry-style) remains available via `setFaultTransport` but is wired on demand.
...
**Resolution completed (2026-06-17).** All three met. (1) CI guard shipped 2026-06-15
(`scripts/version-drift-check.mjs`, negative-tested). (2) CLAUDE.md trimmed from 317→281 lines —
**zero dated `> Consumer vX status` blocks remain** (`grep` clean); the per-version implementation
narrative moved to [`docs/HISTORY.md`](docs/HISTORY.md) (the authoritative changelog stays
`react/CHANGELOG.md`), and the timeless rules distilled into a single **"Architecture conventions
(binding)"** section (money model, budget identity, store slices, sync/faults, Goals/Tax removed,
the views-freeze-columns DB gotcha, zero-cost RPC validation). Also fixed the stale "Current v9.1.01"
overview line → v9.4.3. (3) README gained a **"Documentation map"** table indexing HANDOFF / CLAUDE /
VERSIONS / CHANGELOGs / HISTORY / TECH_DEBT / ARCHITECTURE / DEPLOY / MIGRATIONS / TEST_SCENARIOS —
all links verified to resolve. Drift guard green.
...
| 2026-06-17 | TD-28 (close) | **Doc-architecture closed.** CLAUDE.md trimmed 317→281 (zero dated `> Consumer vX status` blocks; per-version narrative → `docs/HISTORY.md`; timeless rules distilled into one "Architecture conventions (binding)" section; stale "Current v9.1.01" → v9.4.3). README gained a "Documentation map" table (all links verified). The CI version-drift guard (2026-06-15) already prevents version drift recurring. Drift guard green. | Closes **TD-28** (with TD-23). |
| 2026-06-17 | TD-25 (close — crudSlice) | **Entity-CRUD sub-split; closes TD-25.** The repetitive entity writes (budget/allocation/goal/member/debt/asset/account/savedView — pure adapter+set, no money-math) moved verbatim to `store/slices/crudSlice.ts` (186 lines, <300), folded into `Store` via `extends`. The money-critical `upsertTransaction`/`recordDebtPayment` stay in `dataSlice` (now 559 — bootstrap + txn-money + profile, deliberately cohesive). store god-module fully decomposed: `index.ts` 35 + 9 slices. Verified incl. money-model invariants + golden regression; tsc · eslint · 161 tests · build green. | Closes **TD-25**. |
| 2026-06-17 | TD-24 (close) | **Error-opacity closed.** Classified the read-corruption catches (`localStorageCompat.readJson`, `kvStore.kvGet:memory` parse-fails → `expected`) and added the dev diagnostics view (`components/dev/FaultsPanel.tsx` over `getFaults()`, mounted behind `import.meta.env.DEV`). Trivial storage-removeItem/decode/migration noops left intentionally terse per the acceptance. tsc · 161 tests · build green. | Closes **TD-24** (taxonomy + write-loss instrumentation shipped v9.4.0; read-classification + diagnostics here). |
| 2026-06-15 | TD-25 (increment 8 — data core + rename) | **God-module eliminated: data core extracted + `store/index.ts` rename.** `store.ts` was `git mv`'d to `store/slices/dataSlice.ts` (the body — entity state + init/refresh + all CRUD + the money/EMI paths — moved verbatim; only its top/bottom rewritten into a `StateCreator<Store,…>`), and a new 33-line `store/index.ts` is now the pure composition root (`create<Store>` folding all 7 slices + `Store` type + `exposeStoreForE2E`). `useStore`'s public type/behaviour byte-identical; the ~41 `import { useStore } from '../store'` consumers resolve to `store/index.ts` unchanged. Removed a pre-existing dead `splitEmiPortions` import. **Verified byte-identical:** tsc · eslint (only 2 pre-existing `any` warnings) · **161 tests incl. money-model invariants + golden regression** · build — all green. **Residual:** `dataSlice.ts` is 723 lines (the cohesive data core) — splitting its entity CRUD into a `crudSlice` to approach the ~300-line target is the last optional sub-split. | **TD-25** → data core done; ⚠ partial only on the dataSlice line-count. |
| 2026-06-15 | TD-25 (increments 6–7) + housekeeping | **cloudAuthSlice + syncSlice extracted; remote URL fixed.** `store/slices/cloudAuthSlice.ts` (cloudEnabled/session/sessionLoaded/myRole + setSession/refreshHouseholds) and `store/slices/syncSlice.ts` (theme/loading/toasts/lastSyncedAt + manualRefresh/subscribeRealtime/setTheme/toast/dismissToast) moved verbatim, folded into `Store` via `extends`. Removed now-dead `Session`/`AppRole`/`highestRole`/`Theme` + the long-standing pre-existing `getMoneyMapMode` import from store.ts (store.ts now zero lint warnings). Housekeeping: `git remote set-url origin https://github.com/Authen27/Vyact.git` (cleared the Finflow→Vyact redirect). store.ts 861→718; tsc · eslint clean · 161 tests · build green. **Residual:** the data core (init/refresh/all CRUD, ~620 lines) split + the `store.ts`→`store/index.ts` rename — the money/refresh path, sequenced as a dedicated careful pass. | **TD-25** still ⚠ partial. |
| 2026-06-15 | TD-25 (increment 5) | **recurringSlice extracted.** The v7/v8.9 recurring schedules + generation engine (R2 deterministic-id idempotency: `upsertRecurring`/`removeRecurring`/`runRecurringEngine` + state) moved verbatim to `store/slices/recurringSlice.ts`, folded in via `extends RecurringSlice`; persists/generates via the adapter, reads the rest of the store via `get()`. Removed now-dead `dueSchedules`/`generateTransaction`/`advanceSchedule`/`recurringInstanceId` imports from store.ts (kept `backfillSchedulesFromTransactions`). store.ts 958→861. Because it generates transactions, verified incl. money-invariant + golden-regression suites (no money number moved); tsc · eslint · 161 tests · build green. | **TD-25** still ⚠ partial. |
| 2026-06-15 | TD-25 (increment 4) | **notifySlice + shared localJson extracted.** The store-private `readLocalJson`/`readLocalString`/`setLocalJson`/`setLocalString`/`removeLocal` helpers moved to `store/localJson.ts` (so slices can share them without a store cycle); the v7 notification feed + prefs (state + `refreshNotifications`/`markNotificationRead`/`dismissNotification`/`updateNotificationPrefs`) moved verbatim to `store/slices/notifySlice.ts`, folded in via `extends NotifySlice`. Removed now-dead `ls`/`Notification`/`NotificationPrefs` imports from store.ts. store.ts 1,041→958. tsc · 161 tests · build green. | **TD-25** still ⚠ partial. |
