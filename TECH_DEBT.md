...
| TD-25 | ⚠ Consumer `store.ts` god-module → Zustand slices: pattern + modal/testHooks/reconcile/notify/recurring slices + shared localJson extracted *(2026-06-15, 1,167→861)*; cloudAuth/data/sync slices + `index.ts` rename queued | Technical / Maintainability | High | L |
