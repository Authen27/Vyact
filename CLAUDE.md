...
- Consumer: [`react/CHANGELOG.md`](react/CHANGELOG.md) — **current v8.7.1**
...
- **Consumer (React app)** in `react/` — Vite + React 18 + TypeScript + Tailwind + Recharts + Zustand. **Current v8.7.1** (Money-Model Epic 1 + Budgets V2 + Entry-V2 shortForm flag-ON; goals/goalsLens/taxNudge OFF). Supabase cloud (auth, multi-household, invitations, realtime, content module) wired behind the `HybridAdapter`. Local-only mode still works without env vars. **Live (CI-deployed prod): https://vyact-twentyx.vercel.app** — this is the production URL of the `react` project under the `bhushandandolus-projects` Vercel team that `deploy.yml` ships to. ⚠ The older `react-taupe-xi.vercel.app` is **orphaned on a different Vercel account**, not updated by CI, and should not be relied on (it serves a stale build).
...
> **Consumer v8.2.0 status — Money-Model Overhaul (phase 0 of a phased program).** Building [`vyact-money-model-execution-and-regression.md`](vyact-money-model-execution-and-regression.md). Governing principle (Part A): **one source of truth — accounts hold real balances, every transaction moves an account; goals/tax are read-only lenses, never phantom balances; the dashboard is two numbers — Cash Flow (flow) + Net Worth (stock).** Shipped so far (all behind OFF flags = current released behaviour): the program flags in `config/features.ts` (`moneyModel`/`budgetsV2`/`goalsLens`/`taxNudge`/`entryV2`); the mandated regression net (`lib/__tests__/moneyModel.regression.test.ts` — aggregation golden file + transfer-invariant suite); quick wins B2.1/B4.1/B4.4; and (v8.3.0) the **tested engines for all four epics** — `lib/accountBalance.ts` (balance = opening + credits − debits; reconciliation → dated Balance Adjustment; `accounts.opening_balance` DB column + provenance), `lib/budgetIntel.ts` (copy/suggest/history/roll-up, read-only A8), `lib/goalsLens.ts` (virtual vs real-backed goals, R3), `lib/taxNudge.ts` (derived nudge, A5). Store: `reconcileAccount` action + B1.1 cash-default enforcement in `upsertTransaction`. Engine tests `CON-UNIT-MM-100..123`. **Epic 1 is complete + gate-passed (v8.4.0):** the B1.6 account backfill ran on prod after a read-only dry-run (R2 — 0 orphans, global_net invariant), migration `20260607130000`; the Accounts page (`pages/Accounts.tsx`) now shows computed balance + Fix-balance reconcile + per-account ledger, gated by `moneyModel.openingBalance`/`reconciliation`/`ledger`. **UI shipped (v8.4.0/v8.5.0, flag-gated):** Epic 1 account ledger (`pages/Accounts.tsx`), Epic 2 B2.2 history + B2.4 suggest (`pages/Budgets.tsx`), Epic 3 B3.1 lens explainer + B3.2 tax nudge (`pages/Goals.tsx`). **Now built (v8.7.0):** B2.3 budget allocations (`Budget.allocations` + `budgets.allocations` jsonb + sub-limit editor with roll-up in BudgetFormModal, flag `budgetsV2.allocations`) and B4.2/B4.3 txn-form reshape ("+ More details" disclosure for Time/Recurring/Note, flag `entryV2.shortForm`). **Every actionable money-model item is built.** Parked: goal real-account-backing UI (contradicts the v8.6.0 goals removal — `goalsLens` engine ready for when goals return), per-allocation actual-spend bars (need allocation↔category matching), and B4.5 re-theme (PARKED per spec). To go live, flip the relevant flags after a browser QA pass (the app can't be fully run in this environment, though a dev server boots + serves and all modules transform cleanly).
...
> **Goals removed for now (v8.6.0).** `FEATURES.goals.enabled = false` hides the entire goal concept — nav, `/goals` route, Dashboard panel, Add-FAB/`g` shortcut, Ask Vyact chips, and the **Pulse Score's Goal Progress component** (the score renormalises over Budgets/Savings/Trend/Debt via `computePulseScore`'s applicable-weights). Page/modals/store/data are intact and dormant; flip the flag to `true` to fully restore. Gate any NEW goal surface behind `isGoalsEnabled()`.
