...
>   schedule on atomically. **W3 (v10.44.0):** UNDO, corrections, missing-amount and duplicate questions (`whatsapp_pending_turns`). **W4 (v10.45.0):** Ask Vyact answers on WhatsApp with the app's own engine (generated bundle), only with the "Answer my questions here" consent. **W5 (v10.46.0):** Pip (name), one response contract, Wave-1 engine facts, the WhatsApp answer rule. Decided 25 Sep: answers off → Pip offers ANSWERS ON in the chat (never a link); four variant templates with their own images submitted (in review); Reply LOG built; "Name them here" and "Reply UPDATE" were designed on the receptionist canvas. **W6 (v10.47.0):** both built after approval. UPDATE reconciles through the app's own reconcile (`reconcileOnServer`), and naming changes only an Other expense's category. The re-engagement nudges have sender rules, and `reengagement_nudge` was edited in Meta (in review). #120–#123 were merged and deployed 26 Sep.
>
